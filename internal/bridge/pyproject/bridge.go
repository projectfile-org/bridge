...
// Bridge implements core.Syncer for pyproject.toml.
type Bridge struct{}

func (Bridge) Name() string             { return "pyproject" }
...
func (Bridge) Stacks() []string         { return []string{"python"} }
...
func (Bridge) Write(dir string, doc any) error {
	d, ok := doc.(*Document)
	if !ok {
		return fmt.Errorf("pyproject bridge: unexpected document type %T", doc)
	}
	return Write(dir, d)
}
