...
// Bridge implements core.Syncer for package.json.
type Bridge struct{}

func (Bridge) Name() string             { return "npm" }
...
func (Bridge) Stacks() []string         { return []string{"node"} }
...
func (Bridge) Write(dir string, doc any) error {
	d, ok := doc.(*Document)
	if !ok {
		return fmt.Errorf("npm bridge: unexpected document type %T", doc)
	}
	return Write(dir, d)
}
