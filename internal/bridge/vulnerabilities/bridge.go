...
// on-disk filename.
type Bridge struct {
	scanner  string // extKey: "trivy" | "grype" | "osv"
...
func (b Bridge) assemble(pf *projectfile.Document, suppress []pfmodel.VulnerabilitySuppress) []byte {
	...
	buf.WriteString(vulnBanner(b.filename, len(entries)))
...
// edits the projectfile, not the generated file.
func vulnBanner(filename string, count int) string {
	noun := "ID"
	if count != 1 {
		noun = "IDs"
	}
	return fmt.Sprintf(`# %s — generated by `+"`pf-bridge vulnerabilities`"+`
# Source: org.projectfile.vulnerabilities (%d suppressed %s)
# Edit projectfile [org.projectfile.vulnerabilities] suppress to customise —
# direct edits here are overwritten on the next generation.
%s

`, filename, count, noun, core.Marker)
}
