...
func banner(pf *projectfile.Document) string {
	stackList := "(none)"
	if len(pf.Stack) > 0 {
		stackList = strings.Join(pf.Stack, ", ")
	}
...
`, filename, stackList, pfmodel.AttributesExtensionNS, core.Marker)
}
